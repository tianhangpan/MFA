...
class TransTrainer(Utils):
    def __init__(self, args):
        super().__init__()
        print('Initializing trans-trainer ... ')
        ...
        if self.video_dataset_mode.lower() in ['ucsd']:
            _, self.val_list = Utils.get_video_list(Path(self.args.dataset_path),
            ...
            _, self.val_list = Utils.divide_train_list(train_list, self.args.cluster_num)
        self.train_list = Utils.get_trans_list(Path(self.args.dataset_path), self.video_dataset_mode)

        self.mask = self.get_roi_mask(self.args.mask_path)

        if os.path.isfile(self.args.pth_path):
            print(f'loading amb model data from {self.args.pth_path}')
            ...
            raise Exception(f'data not found at {self.args.pth_path}')
